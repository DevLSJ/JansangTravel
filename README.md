...
기말 프로젝트 요구사항에 맞춰 여행 기록 목록은 RecyclerView를 기반으로 구성했습니다.

- Adapter와 ViewHolder 직접 구현
- 여행지명, 방문 날짜, 대표 이미지/썸네일 표시
- 항목 클릭 시 상세 화면으로 이동
- 항목 길게 누르기 시 컨텍스트 메뉴 제공
``class TravelAdapter( private val items: MutableList<TravelRecord>, private val listener: OnTravelClickListener ) : RecyclerView.Adapter<TravelAdapter.TravelViewHolder>()``
